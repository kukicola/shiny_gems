...
RSpec.describe Processing::Workers::SyncAllWorker do
  ...
  before do
    allow(fake_gem_repo).to receive(:pluck_ids_for_hour).with(22).and_return([1, 2, 3])
    allow(fake_repos_repo).to receive(:pluck_ids_for_hour).with(22).and_return([5, 6, 7])
  end
