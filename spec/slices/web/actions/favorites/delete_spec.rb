...
RSpec.describe Web::Actions::Favorites::Delete do
  ...
  let(:fake_favorites_repository) do
    fake_repository(:web, :favorites) do |repository|
      allow(repository).to receive(:unlink).with(gem_id: gem.id, user_id: user.id)
    end
  end
  ...
  it "unlinks favorite" do
    expect(fake_favorites_repository).to receive(:unlink).with(gem_id: gem.id, user_id: user.id)
    subject
  end

  it "sets flash message" do
    expect(subject.flash.next).to eq({success: "Gem removed from favorites"})
  end
end
